...
#def retornar_configuraciones(config: dict):
#    """Extrae configuración de un dict. Usa valores por defecto si faltan claves."""
#    if not config:
#        config = {}
#
#    aciertos = config.get("aciertos", 0)
#    errores = config.get("errores", 0)
#    tiempos_respuesta = config.get("tiempos_respuesta", [])
#    vidas = config.get("vidas_iniciales", 3)
#    puntaje = config.get("puntaje_inicial", 0)
#    monedas = config.get("monedas_iniciales", 0)
#    cantidad_preguntas = config.get("cantidad_preguntas", 5)
#    path_json = config.get("path_json", None)
