...
(PATH_ESTADISTICAS, PATH_PREGUNTAS, PATH_IMAGEN_MENU, PATH_IMAGEN_TIENDA,PATH_IMAGEN_CLASIFICACION, 
  ...
  BUTTON_SPACING_NORMAL, BUTTON_SPACING_PREGUNTA, CANTIDAD_PREGUNTAS,
  VIDAS_INICIALES, PUNTAJE_INICIAL, MONEDAS_INICIALES, ACIERTOS_INICIAL, ERRORES_INICIAL, TIEMPOS_RESPUESTA_INICIAL, COSTO_VIDA) = cargar_todas_constantes(config)
...
def mostrar_pregunta_pygame(screen, pregunta_categoria, categoria):
    ...
    categoria = pregunta_categoria["categoria"]
    ...
    while running:
        ...
        categorias = font_local.render(f"la categoria selecionada es: {categoria}", True, BLACK)
        enunciado_rect = enunciado_texto.get_rect(center=(SCREEN_WIDTH // 2, 150))
        screen.blit(enunciado_texto, enunciado_rect)
        screen.blit(categorias, (SCREEN_WIDTH // 2 - categorias.get_width() // 2, 100))
...
def limitar_preguntas(lista_preguntas, cantidad_config):
    try:
        cantidad = int(cantidad_config)
    except (TypeError, ValueError):
        print("cuidado: CANTIDAD_PREGUNTAS inválida en configuración por tipo de dato; usando todas las preguntas.")
        return lista_preguntas

    if cantidad <= 0:
        print("INFO: CANTIDAD_PREGUNTAS <= 0; 0 preguntas serán jugadas.")
        return lista_preguntas[:0]  # o return lista_preguntas si preferís jugar todas

    # No pedir más de las disponibles
    cantidad = min(cantidad, len(lista_preguntas))
    
    return lista_preguntas[:cantidad]
