...
def jugar_preguntados_pirata(screen):
    ...
    if not preguntas:
        ...
        return
    # Respetar la cantidad de preguntas configurada
    preguntas = limitar_preguntas(preguntas, CANTIDAD_PREGUNTAS)
    
    # Inicializar estadísticas del usuario (desde configuración)
    vidas = VIDAS_INICIALES
    puntaje = PUNTAJE_INICIAL
    aciertos = ACIERTOS_INICIAL
    errores = ERRORES_INICIAL
    monedas = MONEDAS_INICIALES
    tiempos_respuesta = TIEMPOS_RESPUESTA_INICIAL
    ...
    for pregunta in preguntas:
        ...
        respuesta_usuario = mostrar_pregunta_pygame(screen, pregunta,categoria)
    ...
    # Verificar respuesta: ya que la respuesta devuelve un índice (0-3)
        ...
        if vidas > 0 and pregunta != preguntas[-1]:
            monedas, vidas = tienda_de_vidas_pygame(screen, monedas, vidas, costo_vida=COSTO_VIDA,
